...
class TestRadarSeries < Minitest::Test
  def setup
    p = Axlsx::Package.new
  ...
  def test_color
    assert_equal("FF0000", @series.color)
    @series.color = "00FF00"

    assert_equal("00FF00", @series.color)
  end

  def test_ln_width
    assert_nil(@series.ln_width)
    @series.ln_width = "28800"

    assert_equal("28800", @series.ln_width)
  end

  def test_colors
    assert_equal([], @series.colors)
    @series.colors = ["FF0000", "00FF00", "0000FF"]

    assert_equal(["FF0000", "00FF00", "0000FF"], @series.colors)
  end
  ...
  def test_to_xml_string_with_ln_width
    @series.ln_width = "28800"
    doc = Nokogiri::XML(wrap_with_namespaces(@series))

    assert_equal(1, xpath_with_namespaces(doc, "//a:ln[@w='28800']").size)
  end

  def test_to_xml_string_with_ln_width_no_color
    series = @ws.add_chart(Axlsx::RadarChart).add_series(
      data: [0, 1, 2],
      labels: ["zero", "one", "two"],
      ln_width: "28800"
    )
